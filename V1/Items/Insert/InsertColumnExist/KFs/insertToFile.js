...
const StartFunc = ({ LocalCoumnCol1, LocalCoumnCol2 }) => {
  ...
  try {
    if (fs.existsSync(filePath)) {
      ...
      let numberArray = data
        .map(element => Number(element.pk))
        .filter(n => !isNaN(n));

      let MaxPk = Math.max(...numberArray, 0) + 1;

      let LocalInsertData = {
        ItemName: LocalCoumnCol1,
        ItemRate: LocalCoumnCol2,
        pk: MaxPk
      };
      ...
      LocalReturnObject.InsertedObject = LocalInsertData;
